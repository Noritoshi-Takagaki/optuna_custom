...
best_values = []
#乱数ごとの繰り返し
for seeds in range(30):
    try_sampler = TPESampler(seed=seeds,multivariate = True)
    """
...
# 結果の表示
print(f"Average objective value: {np.array(best_values).mean()}")
print(f"Standard deviation: {np.array(best_values).std()}")