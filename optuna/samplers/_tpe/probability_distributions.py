...
class _MixtureOfProductDistribution(NamedTuple):
    ...
    def log_pdf_given_distributions(
        self,
        x: np.ndarray,
        distributions: list[_BatchedDistributions],
    ) -> np.ndarray:
        """
        ユーザーが指定した分布に対し、self.weights に基づく混合分布の対数尤度を返す。
    
        Args:
            x: shape = (batch_size, n_vars)
            distributions: 各変数に対して1つの _BatchedDistributions を持つリスト
                        各 _BatchedDistributions は shape = (n_components,)

        Returns:
            shape = (batch_size,), 各サンプルの混合分布に対する log-pdf
        """
        batch_size, n_vars = x.shape
